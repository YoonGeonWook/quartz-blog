#' Kernel function
#' 
#' @param d Distance b/w center and point
...
#' 설명할/관심 있는 인스턴스들에 대한 샘플의 Euclidean 거리 구하기:
#' @param point_explain Vector of scaled featuares
...
get_y <- function(x1, x2, noise_prob = 0){
  y <- sign(sign(x2 - 1 + abs(x1*2))/3 - sign(x2 - 0.5 + abs(x1*3))/3) + 1
