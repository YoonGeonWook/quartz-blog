get.ycomment.classifier <- function(ycomments){
...
prepare_data <- function(comments, trained_corpus = NULL){
  ...
  if(!is.null(trained_corpus)){
    ...
    labeledTerms <- data.frame(data.table::rbindlist(list(empty_corpus, labeledTerms), fill = T))
...
#' 
#' @param text The text
#' @param pred_fun The prediction function from the ML model.
...
#' @param pred_fun The prediction function from the ML model.
#'                It should contain the complete pipeline: take the raw text, do all the pre-processing
#'                and do the prediction. Retured prediction should be a data.frame with one column per class
...
#' @param class The class for which to create the explanation
explain_text <- function(text, pred_fun, prob = 0.9, n_variations=500, K=3, case=1, class){
  ...
  coefs <- coefs[base::order(abs(coefs), decreasing = T)]
  coefs <- coefs[1:K]
